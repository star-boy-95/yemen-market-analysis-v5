...
@handle_errors
def create_weights(data: gpd.GeoDataFrame, weight_type: str = 'distance', **kwargs) -> weights.W:
    """
    Create spatial weights for a subset of data.

    Args:
        data: GeoDataFrame containing spatial data.
        weight_type: Type of weights to create. Options are 'distance', 'contiguity', 'kernel'.
        **kwargs: Additional parameters for the weight creation.
    ...
    """
    logger.info(f"Creating {weight_type} weights for subset of data")

    # Create weight matrix object
    weight_matrix = SpatialWeightMatrix(data)

    # Create weights based on type
    if weight_type == 'distance':
        k = kwargs.get('k', 4)
        p = kwargs.get('p', 2.0)
        alpha = kwargs.get('alpha', -1.0)
        threshold = kwargs.get('threshold', None)
        return weight_matrix.create_distance_weights(k=k, p=p, alpha=alpha, threshold=threshold)
    elif weight_type == 'contiguity':
        queen = kwargs.get('queen', True)
        return weight_matrix.create_contiguity_weights(queen=queen)
    elif weight_type == 'kernel':
        bandwidth = kwargs.get('bandwidth', None)
        kernel = kwargs.get('kernel', 'gaussian')
        fixed = kwargs.get('fixed', True)
        k = kwargs.get('k', 4)
        return weight_matrix.create_kernel_weights(bandwidth=bandwidth, kernel=kernel, fixed=fixed, k=k)
    else:
...
class SpatialWeightMatrix:
    ...
    def create_contiguity_weights(
        ...
        try:
            # Due to compatibility issues with NumPy 2.0 and libpysal's voronoi implementation,
            # we'll use a distance-based approach instead of contiguity
            logger.info("Using distance-based weights as a fallback for contiguity weights")
            ...
            # Create KNN weights with k=4 (approximates contiguity for most cases)
            k = 4
            self.w = weights.KNN.from_array(coords, k=k)

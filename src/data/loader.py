...
class DataLoader:
    ...
    def filter_by_commodity(
        ...
        # Filter data
        filtered_data = data[data['commodity'] == commodity].copy()
    ...
    def filter_by_markets(
        ...
        # Filter data
        filtered_data = data[data['market'].isin(markets)].copy()
    ...
    def _split_by_market(self, data: gpd.GeoDataFrame) -> Dict[str, pd.DataFrame]:
        ...
        # Check if market column exists, if not use admin1 as market
        market_column = 'market'
        if 'market' not in data.columns:
            if 'admin1' in data.columns:
                logger.info("Using 'admin1' column as market identifier")
                market_column = 'admin1'
                # Create a copy of the data and add a market column
                data = data.copy()
                data['market'] = data['admin1']
            else:
                logger.error("Neither 'market' nor 'admin1' column found in data")
                raise YemenAnalysisError("Neither 'market' nor 'admin1' column found in data")
        ...
        for market in markets:
            market_df = data[data[market_column] == market].copy()
